...
def run_eval(cfg: ESMConfig) -> None:
    ...
    from esm.online.greedy import run_greedy_dataset, run_self_consist_dataset, run_self_discover_dataset

    methods = [str(m).lower().replace("-", "_") for m in cfg.eval.methods]
    ...
    try:
        ...
        if "self_consist" in methods:
            tags["self_consist"] = f"self_consist_T{T}"
            acc_at_T["Self-Consist"] = float(
                run_self_consist_dataset(cfg, max_new_tokens=T, out_tag=tags["self_consist"], llm=llm)
            )
        if "self_discover" in methods:
            tags["self_discover"] = f"self_discover_T{T}"
            acc_at_T["Self-Discover"] = float(
                run_self_discover_dataset(cfg, max_new_tokens=T, out_tag=tags["self_discover"], llm=llm)
            )
        if "esm" in methods:
            tags["esm"] = f"ESM_T{T}"
