...
from utils.prompt_templates import build_chat_messages, get_answer_prompt_config
...
def parse_args():
    ...
    parser.add_argument("--max_eval", type=int, default=None, help="Limit number of evaluation samples to match baseline subsets")
    parser.add_argument("--dtype", type=str, default="bfloat16")
    parser.add_argument("--max-model-len", "--model-context-len", type=int, default=None, dest="model_context_len") # max-model-len for vllm, should be longer than max_generated_tokens.
    ...
    parser.add_argument("--split", type=str, default="test") # dataset split
...
def main():
    ...
    if not os.path.isabs(args.output_path):
        args.output_path = os.path.join(SCRIPT_ROOT, args.output_path)
    if args.model_context_len is None:
    ...
    try:
        questions_json = load_data(args.dataset, args.split, args.dataset_dir)
        if not questions_json:
            print(f"Error: No questions loaded for dataset {args.dataset} split {args.split}.")
            sys.exit(1)
        if args.max_eval is not None:
            questions_json = questions_json[:args.max_eval]
        print(f"Successfully loaded dataset: {args.dataset} ({args.split}), total {len(questions_json)} questions" + (f" (capped to {args.max_eval})" if args.max_eval is not None else ""))
    except Exception as e:
    ...
    answer_prompt_str, pred_prob_stop_tokens = get_answer_prompt_config(args.dataset) # Prompt string to guide answer generation
    ...
    pred_prob_stop_tokens = list(set(pred_prob_stop_tokens + ['</think>']))
    ...
    for i, question_data in enumerate(questions_json):
        messages = build_chat_messages(question_data, args.dataset)
        formatted_prompt = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        example_idx = question_data.get('idx', i)
        ...
        for q_idx in current_active_indices_for_batching:
            ...
            try:
                ...
                if remaining_context_window <= 0:
                     ...
                     question_text = state['question_data'].get('problem', state['question_data'].get('question', ''))
                     output_idx = state.get('example_idx', state.get('question_index', q_idx))
                     state['output_dict'] = {'idx': output_idx, 'question': question_text, 'generated_responses': [state['generated_thinking_history'] + state['generated_answer_history'] + "\n" + state['error_message']], 'gold_answer': state['question_data']['answer']}
            ...
            try:
                ...
                if step_type == 'think':
                    ...
                    if last_token_id != tokenizer.eos_token_id and args.af == 1:
                        ...
                        question_text = state['question_data'].get('problem', state['question_data'].get('question', ''))
                        output_idx = state.get('example_idx', state.get('question_index', q_idx))
                        state['output_dict'] = {'idx': output_idx, 'question': question_text, 'generated_responses': [final_response_text], 'gold_answer': state['question_data']['answer'], 'too_long': state['too_long'], 'thinking_steps': state['thinking_steps'], 'rep_end': state['rep_end'], 'high_prob': state['high_prob'],'regular_end': state['regular_end']}
                        if q_idx in active_questions_indices:
                            active_questions_indices.remove(q_idx)
                            pbar.update(1)
    ...
    final_results.sort(key=lambda x: x.get('idx', len(questions_json)))
